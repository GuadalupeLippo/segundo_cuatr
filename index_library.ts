...
class Library {
  ...
  makebook(book:string, arrayBook: Array <Book>) :
void{
    let propiedadesBook: string[] = book.split(`,`);
    let id : string = uid()
    id = propiedadesBook[0]
    let title: string = propiedadesBook[1]
    let year: number = Number(propiedadesBook[2])
    let author: string = propiedadesBook[3]
    let newBook: Book = new Book( title, year, author)
